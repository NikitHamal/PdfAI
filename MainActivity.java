...
public class MainActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ImageView apiLogIcon = findViewById(R.id.api_log_icon);
        ...
        apiLogIcon.setOnClickListener(v -> startActivity(new Intent(this, ApiLogActivity.class)));
    ...
    private void showSettingsDialog() {
        dialogManager.showSettingsDialog(geminiApiKey, selectedModel, (apiKey, model) -> {
            geminiApiKey = apiKey;
            selectedModel = model;
        });
    }
    ...
    private void callGeminiForOutline(String userPrompt) {
        ...
        geminiApiClient.generateOutline(userPrompt, geminiApiKey, selectedModel, new GeminiApiClient.GeminiApiCallback() {
            ...
            public void onSuccess(String response) {
                apiLogManager.addLog(userPrompt, response);
                runOnUiThread(() -> {
                    try {
            ...
            public void onFailure(String error) {
                apiLogManager.addLog(userPrompt, error);
                runOnUiThread(() -> {
    ...
    private void generateSectionContent(OutlineData outlineData, int sectionIndex) {
        ...
        geminiApiClient.generateSectionContent(outlineData.getPdfTitle(), sectionTitle, outlineData.getSections(), sectionIndex, geminiApiKey, selectedModel, new GeminiApiClient.GeminiApiCallback() {
            @Override
            public void onSuccess(String response) {
                apiLogManager.addLog("Generate content for: " + sectionTitle, response);
                runOnUiThread(() -> {
                    try {
                        JSONObject jsonResponse = new JSONObject(response);
            ...
            @Override
            public void onFailure(String error) {
                apiLogManager.addLog("Generate content for: " + sectionTitle, error);
                runOnUiThread(() -> {
